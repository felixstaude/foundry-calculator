...
function CardNode({ data }: NodeProps<GraphNode>) {
  const borderColor =
    data.isTarget && data.color
      ? `border-[${data.color}] shadow-[0_0_12px_${data.color}]`
      : data.isTarget
  ...
  return (
    <div
      className={classNames('w-[240px] rounded-lg border bg-slate-900/90 p-3 shadow-lg shadow-slate-900/40', borderColor)}
      style={data.color ? { borderLeft: `4px solid ${data.color}` } : undefined}
    >
...
function FlowEdge({ id, sourceX, sourceY, targetX, targetY, data, markerEnd, sourcePosition, targetPosition }: EdgeProps<FlowEdgeData>) {
  ...
  return (
    <>
      ...
      />
      <foreignObject x={labelX - 40} y={labelY - 14} width={80} height={24} pointerEvents="none">
        <div className="flex w-full items-center justify-center">
          <span className="rounded-full bg-slate-900/80 px-2 py-0.5 text-[10px] font-semibold text-slate-100 shadow-sm shadow-black/30">
            {data?.throughput ? `${formatDisplay(data.throughput)} / min` : data?.label}
          </span>
        </div>
      </foreignObject>
...
function ProductionGraph({
  ...
  const computeLayout = () => {
    ...
    const rfEdges = bundled.map((e) => {
      ...
      const color = (graphNodes.find((g) => g.id === e.from)?.color as string) ?? lanePalette[laneIndex] ?? '#818cf8';
